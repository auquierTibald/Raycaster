...
namespace Settings {
    ...
    float ps = 0.003; // Player's speed
    float pruns = 0.006; // Player's running speed
    float pss = 0.003; // Player's strafe speed
    ...
    const int num_textures = 5; // Number of textures used in the game

    const float mouse_sensivity = 0.05f;

    const long num_sprite = 2; // Number of sprites in the game
    const long num_animated_sprite = 0;
    const long num_npc = 2;

    const long max_animation_frames = 10;
    const long max_animations = 10;

    int raycasting_textures = 4;
