...
namespace Raycasting {
    bool in(int number, int indexes[Settings::num_sprite])
    {
        bool flag = false;
        for (int i = 0; i < Settings::num_sprite; i++)
        {
            if(indexes[i] == number)
            {
                flag = true;
    ...
    public:
        ...
        vector<float> rangles, texturesoffset; // Ray end coordinates
        float ox, oy;
        ...
        void projection(float playerangle, float pz)
        {
            ...
            for (int ray = 0; ray < Settings::num_rays; ray++)
            {
                ...
                SDL_Rect col = {projX[ray], (int)(hor_offset - pz), Settings::colW, (int)proj_height};
                SDL_Rect dest = {(int)(texturesoffset[ray] * Settings::texture_size), 0, 1, Settings::texture_size};
                if(raytiles[ray] != 0)
                {
                    destRect[ray] = dest;
                    srcRect[ray] = col;
