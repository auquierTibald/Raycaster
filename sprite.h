...
namespace Sprite {
    ...
    class Sprite{
        public:
            ...
            bool isStatic = true, onscreen = false;
            ...
            void draw(double depths[Settings::num_rays], SDL_Texture* texture[Settings::num_rays], SDL_Rect destRect[Settings::num_rays], SDL_Rect srcRect[Settings::num_rays], bool isSprite[Settings::num_rays])
            {
                ...
                if(onscreen)
                {
                    ...
                    int ray; bool flag = false;
                    for(int x = -2; x < w+2; x++)
                    {
                        ray = (int)((posx + x * Settings::colW) / Settings::colW);
                        if(ray > 0 && ray < Settings::num_rays && isSprite[ray] && depths[ray] < dist) {flag = true;}
                    }
                    if(!flag)
                    {
                    ...
                    for(int x = -2; x < w+2; x++)
                    {
                        ...
                        else if(ray > 0 && ray < Settings::num_rays)
                        {
                            depths[ray] = dist;
                            isSprite[ray] = true;
                        }
    ...
    class NPC : public AnimatedSprite{
        ...
        bool isAlive = true;
        bool pain = false;
        double attack_dist = 3, damage_dist = 2;
        ...
        void update(double deltaTime, int map[Settings::mapHeight][Settings::mapWidth], double depths[Settings::num_rays])
        {
            ...
            if(this->dist > damage_dist && this->dist < attack_dist && isAlive)
            {
                this->current = 1;
                srand(time(0));
            ...
            else if(this->dist < damage_dist && isAlive)
            {
                if(player->attacking)
                {
