...
public:
    ...
    int currentmap[Settings::mapHeight][Settings::mapWidth];
    ...
    void setup() {
        ...
        player->weaponanimation[0] = TextureLoader::loadTexture(renderer, "assets/sword1.bmp");
        player->weaponanimation[1] = TextureLoader::loadTexture(renderer, "assets/sword2.bmp");
        player->weaponanimation[2] = TextureLoader::loadTexture(renderer, "assets/sword3.bmp");
        player->weaponanimation[3] = TextureLoader::loadTexture(renderer, "assets/sword4.bmp");  
        player->currentweaponframe = player->weaponanimation[0];
    ...
    void editorinputs(SDL_Event event)
    {
        ...
        if(event.type == SDL_MOUSEBUTTONDOWN)
        {
            ...
            {   
                ...
                {
                    if(currentTexture >= Settings::raycasting_textures)
                    {
                        currentTexture = 0;
                    }
                    else
    ...
    void editordraw()
    {
        ...
        else if(objectType == 1) {SDL_RenderCopy(renderer, TextureLoader::loadTexture(renderer, "assets/tree.bmp"), NULL, &current);}
        else if(objectType == 2) {SDL_RenderCopy(renderer, TextureLoader::loadTexture(renderer, "assets/plaexisbrusier_spritesheet.bmp"), &src1, &current);}
    ...
    void render() {
        ...
        else if(gameState == 1)
        {
            ...
            raycasting->drawfloor(3, player->x, player->y, player->zOffset, player->angle);
            raycasting->projection(player->angle, player->zOffset);
